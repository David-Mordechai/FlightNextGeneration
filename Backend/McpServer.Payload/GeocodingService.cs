    ...
    public async Task<(double Lat, double Lng, double Alt)?> GetCoordinatesAsync(string locationName)
    {
        try
        {
            ...
            if (points != null)
            {
                ...
                if (match == null)
                {
                    match = points.FirstOrDefault(p => p.Name.Equals($"Home {locationName}", StringComparison.OrdinalIgnoreCase));
                }

                // 4. Try contains (Fuzzy)
                if (match == null)
                {
                    match = points.FirstOrDefault(p => p.Name.Contains(locationName, StringComparison.OrdinalIgnoreCase));
                }
